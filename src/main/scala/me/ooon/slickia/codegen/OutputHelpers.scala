...
trait OutputHelpers {
  ...
  /** Generates code and writes it to a file. Creates a folder structure for the given package inside the given
    * srcFolder and places the new file inside or overrides the existing one.
    * @group Output
    * @param folder
  ...
  def writeToMultipleFiles(profile: String, folder: String, pkg: String, container: String = "Tables"): Unit = {
    ...
    codePerTable.foreach { case (tableName, tableCode) =>
      writeStringToFile(packageTableCode(tableName, tableCode, pkg, container),
                        folder,
                        pkg,
