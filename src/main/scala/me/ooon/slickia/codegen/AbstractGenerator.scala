...
//    final lazy val columnsPositional: IndexedSeq[Column] = model.columns.map(Column).toIndexedSeq
    final lazy val columnsPositional: IndexedSeq[Column] = model.columns.map { c =>
      var c0 = c

      // pg serial
      if (
        !c0.options.forall {
          case slick.sql.SqlProfile.ColumnOption.SqlType(x) if x == "serial" => false
          case _                                                             => true
        }
      ) {
        c0 = c0.copy(nullable = true)
      }

      Column(c0)
    }.toIndexedSeq

    /** Database column positions in the desired user-facing order. Currently just moves the positions of AutoInc
      * columns to the end if autoIncLastAsOption is enabled.
      */
    ...
    /** Indicates whether auto increment columns should be put last and made an Option with a None default. Please set
      * to !hlistEnabled for switching this on.
      * @group Basic
      * customization overrides
      */
    ...
    /** Indicates if this table should be mapped using factory and extractor or not, in which case tuples are used.
      * (Consider overriding EntityType.enabled instead, which affects this, too.) Disabled by default when using
      * hlists.
      * @group Basic
      * customization overrides
      */
    ...
    abstract case class ColumnDef(val model: m.Column) extends TermDef {
      ...
      def actualType: Code = if (model.nullable) optionType(rawType) else rawType
      ...
      /** Indicates whether this column should be user facing as a nullable column with default None even though it is
        * not. Useful for autoInc columns.
        */
      ...
      def autoInc = model.options.contains(ColumnOption.AutoInc)
