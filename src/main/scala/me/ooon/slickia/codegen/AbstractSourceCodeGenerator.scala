...
abstract class AbstractSourceCodeGenerator(model: m.Model) extends AbstractGenerator[String, String, String](model) with StringGeneratorHelpers with StrictLogging{
  ...
  def foreignKeysPerTable: Map[String, List[String]] =
    tables.map(t => t.TableValue.name -> t.model.foreignKeys.map(k => tableName(k.referencedTable.table)).toList).toMap
  ...
  ${indent(body)}
    ...
    trait TableClassDef extends super.TableClassDef {
      def star = {
        ...
        val rhs =
          if (isMappedToHugeClass) s"($struct).mapTo[${typeName(entityName(model.name.table))}]"
          else if (mappingEnabled) s"$struct <> ($factory, $extractor)"
          else struct
        s"def * = $rhs"
      }
      def option = {
      ...
      def optionFactory = {
        ...
        val expr =
          if (isMappedToHugeClass)
            ...
            val discriminator = columns.zipWithIndex.collectFirst {
              case (c, i) if !c.model.nullable => if (columns.size > 1 && !isMappedToHugeClass) tuple(i) else "r"
            }
